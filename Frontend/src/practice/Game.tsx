            ...
            {revealed && (
              <div className="col g14 fade" role="status" aria-live="polite">
                <div className="verdict" data-s={pick === scenario.c ? (lastEntry?.fast ? 'ok' : 'slow') : 'bad'}>
                  ...
                  <span>{pick === null
                    ? `In the real test hesitation is scored as a failure to decide. ${DECISION_LIMIT_MS / 1000} seconds is the window you get.`
                    : pick === scenario.c && !lastEntry?.fast
                      ? `You took ${(lastEntry.ms / 1000).toFixed(1)}s. Under ${FAST_ANSWER_MS / 1000} seconds is what a confident driver looks like.`
