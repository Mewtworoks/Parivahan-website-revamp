...
import { AXIS_LABELS, DECISION_LIMIT_MS, FAST_ANSWER_MS, SCENARIOS } from './scenarios';
...
/** The 12-situation practice round: pick an answer against a 4-second countdown, see why, move on. */
export function Game({ go, state, update }: PageProps) {
  const focusAxis = state.focus || null;
  const queue = useMemo(() => {
    const all = SCENARIOS.slice();
  ...
  function commit(choice: number | null) {
    ...
    if (correct) setScore(v => v + (fast ? 120 : 70));
    else { setHearts(h => h - 1); setShake(true); setTimeout(() => setShake(false), 420); }
    setPick(choice);
    setRevealed(true);
  }
  ...
  function goToNext() {
    ...
    else { setIndex(index + 1); setPick(null); setRevealed(false); locked.current = false; window.scrollTo(0, 0); }
  }
  ...
  return (
    <div className="narrow fade" style={{ padding: '28px 24px 0', maxWidth: 820 }}>
      <div className="gamecard col">
        <div className="hud">
          <div className="row g10 center">
            <span className="hearts">{[0, 1, 2].map(n => <i key={n} data-on={n < hearts ? '1' : null} />)}</span>
            <span className="hud-l hide-m">{AXIS_LABELS[scenario.lvl]}</span>
          </div>
          <div className="row g16 center">
        ...
        <div className="col g20" style={{ padding: '22px 24px 24px' }}>
          ...
          {revealed && (
            <div className="col g14 fade">
              ...
              <button className="btn btn-p" onClick={goToNext}>{hearts <= 0 ? 'See my report card' : index >= queue.length - 1 ? 'Finish and see report card' : 'Next situation'} {Icon.right()}</button>
            </div>
          )}
          {!revealed && (
