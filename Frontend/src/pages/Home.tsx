...
const JOURNEY_STEPS: [category: string, categoryHi: string, categoryMr: string, heading: string, headingHi: string, headingMr: string, body: string, bodyHi: string, bodyMr: string, tag: string, tagHi: string, tagMr: string][] = [
  ['Eligibility', 'पात्रता', 'पात्रता', 'Are you eligible', 'क्या आप पात्र हैं', 'तुम्ही पात्र आहात का',
    ...
    'Instant', 'तुरंत', 'लगेच'],
  ['Application', 'आवेदन', 'अर्ज', 'Fill the form once', 'फ़ॉर्म एक बार भरें', 'फॉर्म एकदा भरा',
        ...
        <div className="wrap">
          <div className="in">
            ...
            <div className="hero-art hide-m">
              {/* Decorative only — every field on the mocked-up card is a
                  greyed-out bar, not real text, so there is no identity here
                  to mask. */}
              <HeroScene />
            </div>
          </div>
        ...
        <div className="steps-card">
          ...
          <div className="steps">
            {JOURNEY_STEPS.map(([category, categoryHi, categoryMr, heading, headingHi, headingMr, body, bodyHi, bodyMr, tag, tagHi, tagMr], n) => (
              <div className="step-card" key={heading} data-reveal>
                <div className="step-card-top">
                  <span className="step-ic">{JOURNEY_ICONS[n]({ width: 22, height: 22 })}</span>
                  <span className="step-badge">{String(n + 1).padStart(2, '0')}</span>
                </div>
                <span className="step-cat">{t(category, categoryHi, categoryMr)}</span>
                <h3>{t(heading, headingHi, headingMr)}</h3>
                <p>{t(body, bodyHi, bodyMr)}</p>
                <div className="step-card-foot">
                  <span>{t('Step', 'चरण', 'टप्पा')} {n + 1}/4</span>
                  <span className="step-tag">{t(tag, tagHi, tagMr)} {Icon.right({ width: 12, height: 12 })}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="wrap" style={{ marginTop: 44 }} data-reveal>
        <div className="card col" style={{ overflow: 'hidden' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) minmax(0,1.1fr)', alignItems: 'stretch' }} className="learnband">
            ...
            {/* The actual game screen, not a decorative stand-in — RoadScene
                is what every one of the 29 scenarios now renders during real
                play (PixelScene is dead-fallback code kept for a spec that
                should never be missing), so this preview shows exactly what
                pressing "Play the road" opens, smoothed edges included. */}
            <div style={{ minHeight: 230 }}><RoadScene spec={ROAD_SPECS[SCENARIOS[4].id] || ROAD_SPECS.H2} /></div>
          </div>
        </div>
      </section>
      <section className="wrap" style={{ marginTop: 44 }} data-reveal>
