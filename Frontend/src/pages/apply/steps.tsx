    ...
    <div className="col g20">
      ...
      <div className="card card-p col g14">
        ...
        <Note>{stateName === 'Maharashtra' || stateName === 'Bihar'
          ? <span>{stateName === 'Bihar' ? <span>Bihar is modelled here as the contrast case. Same eight stages, but the state adds three charges of its own — you will see each one named. </span> : null}Applying with Aadhaar exempts you from three of these stages, and you will see which and why before you submit.</span>
          : <span>{stateName} is not modelled in this prototype. The stages are identical nationally, but fees, RTO lists and document requirements vary by state. Maharashtra and Bihar are the two with real data behind them here.</span>}</Note>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="col g20">
      ...
      <div className="card card-p col g16">
        ...
        {form.route === 'manual' && <div className="fade"><Note tone="warn">That means a visit to the RTO. It is a legitimate choice and we will not push you off it — but you should know the cost before you pick, not after. You can switch on this screen at any time.</Note></div>}
        {form.route === 'aadhaar' && <div className="fade"><Note tone="brand">Exempted for you: upload documents, upload photo, and slot booking. The test password comes to your Aadhaar-registered mobile by SMS and you take the test wherever you are.</Note></div>}
      </div>
    </div>
  );
}
  ...
  return (
    <div className="col g20">
      ...
      <div className="card card-p col g14">
        ...
        {extraFees.length > 0 && <Note tone="warn">{t(`${stateName} adds ${extraFees.length} charges of its own`, `${stateName} अपने ${extraFees.length} शुल्क जोड़ता है`, `${stateName} स्वतःचे ${extraFees.length} शुल्क जोडते`)} — {extraFees.map(x => x.k.toLowerCase()).join(', ')}. {t(`They are legitimate state levies, but on the official portal they appear for the first time on the payment page, which is why the same licence costs ₹350 in Maharashtra and ₹740 here. Every line is named on the next screen.`, `यह वैध राज्य शुल्क हैं, लेकिन आधिकारिक पोर्टल पर यह पहली बार भुगतान पेज पर दिखते हैं, इसीलिए वही लाइसेंस महाराष्ट्र में ₹350 और यहां ₹740 का पड़ता है। अगली स्क्रीन पर हर लाइन का नाम है।`, `हे वैध राज्य शुल्क आहेत, पण अधिकृत पोर्टलवर ती पहिल्यांदा पेमेंट पानावर दिसतात, म्हणूनच तेच लायसन्स महाराष्ट्रात ₹350 आणि इथे ₹740 चे पडते. पुढील स्क्रीनवर प्रत्येक ओळीचे नाव दिलेले आहे.`)}</Note>}
              ...
              <div key={id} className="col g8">
                ...
                {status === 'warn' && (
                  <Note tone="warn">{id === 'photo'
                    ? <span>{t('Checked instantly: the top of your head is cut off.', 'तुरंत जांचा गया: आपके सिर का ऊपरी हिस्सा कट गया है।', 'त्वरित तपासले: तुमच्या डोक्याचा वरचा भाग कापला गेला आहे.')} {t('Hold the camera further away. The counter would have sent you back for this.', 'कैमरे को और दूर रखें। काउंटर पर इसके लिए आपको वापस भेज दिया जाता।', 'कॅमेरा आणखी दूर ठेवा. काउंटरवर यासाठी तुम्हाला परत पाठवले असते.')}</span>
                    : <span>{t('Checked instantly: the signature runs off the edge of the paper.', 'तुरंत जांचा गया: हस्ताक्षर कागज़ के किनारे से बाहर जा रहा है।', 'त्वरित तपासले: स्वाक्षरी कागदाच्या कडेबाहेर जाते आहे.')} {t('Sign in the middle and shoot straight down.', 'बीच में हस्ताक्षर करें और सीधा ऊपर से फोटो लें।', 'मध्ये स्वाक्षरी करा आणि सरळ वरून फोटो घ्या.')}</span>}</Note>
                )}
